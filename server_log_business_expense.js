...
tableHeader='<tr><th>Cost<th>Payee<th>Description<th>Link'//https://www.w3.org/TR/html401/struct/tables.html
function returnTableRowString(post,milliseconds) {//e.g. use as returnTableRowString(post,Date.now())
	return `<tr><td>${post.cost}<td>${post.payee}<td>${post.description}<td><a href=${post.link}>${post.link}</a></table>`}
